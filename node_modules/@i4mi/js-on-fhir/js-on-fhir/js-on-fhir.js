...
exports.JSOnFhir = void 0;
var fhir_r4_1 = require("@i4mi/fhir_r4");
var JSOnFhir = /** @class */ (function () {
    /**
    ...
    *                         have to be set.)
    */
    function JSOnFhir(serverUrl, clientId, redirectUrl, doesNotNeedAuth) {
        this.apiMethods = new fhir_r4_1.ApiMethods();
        this.urls = {
            service: '',
            conformance: '',
            auth: '',
        ...
        var persisted = JSON.parse(sessionStorage.getItem('jsOnFhir'));
        if (persisted != null) {
            if (persisted.urls.redirect === redirectUrl && persisted.urls.service === serverUrl + '/fhir' && persisted.settings.client === clientId) {
                ...
                persisted.apiMethods = new fhir_r4_1.ApiMethods();
                return persisted;
            }
        }
    ...
    */
    JSOnFhir.prototype.authenticate = function (params) {
        var _this = this;
        ...
        this.fetchConformanceStatement()
            .then(function (res) {
            // generate auth url
            var authUrl = _this.urls.auth + '?' +
                'response_type=' + _this.settings.responseType + '&' +
                "client_id=" + encodeURIComponent(_this.settings.client) + "&" +
                "scope=" + encodeURIComponent(_this.settings.scope) + "&" +
                "redirect_uri=" + encodeURIComponent(_this.urls.redirect) + "&" +
                "aud=" + encodeURIComponent(_this.urls.service) + "&" +
                "state=" + _this.generateRandomState(64);
            if (_this.settings.language.length > 0) {
                authUrl += '&language=' + _this.settings.language;
            }
            if (params) {
    ...
    JSOnFhir.prototype.handleAuthResponse = function () {
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (paramString.includes('state=') && paramString.includes('code=')) {
                ...
                if (_this.settings.state === state) {
                    var data = 'grant_type=' + 'authorization_code'
                        + '&redirect_uri=' + encodeURIComponent(_this.urls.redirect)
                        + '&client_id=' + encodeURIComponent(_this.settings.client)
                        + '&code=' + code;
                    (0, fhir_r4_1.apiCall)({
                        url: _this.urls.token,
                        method: fhir_r4_1.HttpMethod.POST,
                        headers: {
                        ...
                        if (response.status === 200) {
                            _this.interpretAuthResponse(response);
                            // reset the url so we don't run into misinterpreting the same code later again
                            history.pushState({}, null, _this.urls.redirect);
    ...
    *                             an empty promise
    */
    JSOnFhir.prototype.refreshAuth = function (refreshToken) {
        var _this = this;
        if (this.settings.noAuth) {
    ...
    */
    JSOnFhir.prototype.create = function (resource) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            if (!(_this.isLoggedIn() || _this.settings.noAuth)) {
                reject('Not logged in');
            }
            var config = {
                access_token: _this.auth.token,
                authorization_type: _this.auth.type,
                base_url: _this.urls.service
            };
            // call create of apimethods
            _this.apiMethods.create(resource, config).then(function (response) {
    ...
    * @returns reject every other case with message
    */
    JSOnFhir.prototype.update = function (resource) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            ...
            _this.apiMethods.update(resource, config).then(function (response) {
                if (response.status === 200 || response.status === 201)
                    resolve(JSON.parse(response.body));
                else
                    reject(response);
    ...
    * @returns reject every other case with message
    */
    JSOnFhir.prototype.search = function (resourceType, params) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            ...
            _this.apiMethods.search(params, resourceType, config).then(function (response) {
                if (response.status === 200 || response.status === 201) {
                    resolve(JSON.parse(response.body));
                }
                else {
    ...
    */
    JSOnFhir.prototype.processMessage = function (message) {
        ...
        if (!message.entry.find(function (entry) { return entry.resource.resourceType === 'MessageHeader'; })) {
    ...
    * @returns reject every other case with message
    */
    JSOnFhir.prototype.performOperation = function (operation, payload, httpMethod, params, resourceType, resourceId) {
        var _this = this;
        if (httpMethod === void 0) { httpMethod = fhir_r4_1.HttpMethod.POST; }
        ...
        return new Promise(function (resolve, reject) {
            (0, fhir_r4_1.apiCall)({
                url: _this.urls.service + resourceType + resourceId + '/$' + operation + paramUrl,
                method: httpMethod,
                headers: {
    ...
    */
    JSOnFhir.prototype.setLanguage = function (lang) {
        if (lang.length === 2) {
    ...
    */
    JSOnFhir.prototype.setConformanceUrl = function (conformanceUrl) {
    ...
    */
    JSOnFhir.prototype.setScope = function (scope) {
        this.settings.scope = '';
        this.persistMe();
    };
    /**
    ...
    */
    JSOnFhir.prototype.getPatient = function () {
    ...
    */
    JSOnFhir.prototype.generateRandomState = function (length) {
        if (length <= 0) {
            length = 122;
        }
    ...
    */
    JSOnFhir.prototype.fetchConformanceStatement = function () {
        var _this = this;
        return new Promise(function (resolve, reject) {
            var cfUrl = (typeof _this.urls.conformance !== 'undefined') ? _this.urls.conformance : _this.urls.service + '/metadata';
            (0, fhir_r4_1.apiCall)({
                url: cfUrl,
                method: fhir_r4_1.HttpMethod.GET
            }).then(function (response) {
                return _this.interpretConformanceStatementResponse(response);
    ...
    */
    JSOnFhir.prototype.getAccessToken = function () {
    ...
    */
    JSOnFhir.prototype.interpretConformanceStatementResponse = function (response) {
        if (response.status === 200) {
    ...
    */
    JSOnFhir.prototype.interpretAuthResponse = function (response) {
...
exports.JSOnFhir = JSOnFhir;
//# sourceMappingURL=js-on-fhir.js.map